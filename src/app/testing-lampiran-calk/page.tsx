...
import { BabCalk, SubbabCalk } from "../_types/types";
import { generateDaftarIsiCALK } from "../_utils/generate-daftar-isi-calk";

interface CalkStructureEditorProps {
  initialData?: BabCalk[];
  onSave: (data: BabCalk[]) => void;
}
...
export default function CalkStructureEditor({
  ...
  /* ✅ Simpan Struktur */
  const handleSave = () => {
    for (const [i, b] of babs.entries()) {
      if (!b.judul) return alert(`Lengkapi judul untuk BAB ${b.bab}`);
      if (b.halamanMulai > 206)
        return alert(`BAB ${b.bab} tidak boleh melebihi halaman 206`);
      for (const [j, s] of (b.subbabs || []).entries()) {
        if (!s.judul)
          return alert(`Lengkapi judul subbab ${b.bab}.${s.subbab}`);
        if (s.halamanMulai > 206)
          return alert(
            `Halaman subbab ${b.bab}.${s.subbab} melebihi batas (206)`
          );
      }
    }
